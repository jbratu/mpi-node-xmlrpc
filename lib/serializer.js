...
exports.serializeMethodCall = function(method, params, encoding) {
  ...
  // Incoming parameters will have a header and parameters property
  let paramsReplaced = webconnectReplaceStringsSharedObject(params.header);
  serializeValue(paramsReplaced, xml.ele('param'));

  paramsReplaced = webconnectReplaceStringsSharedObject(params.parameters);
  serializeValue(paramsReplaced, xml.ele('param'));
...
function webconnectReplaceStringsSharedObject(inputObject) {
  const linefeeds = ["\r\n", "\n", "\r", "\n\r"];

  // Helper function to process strings and nested objects
  const processValue = (value) => {
    if (typeof value === 'object' && value !== null) {
      return webconnectReplaceStringsSharedObject(value); // Recursive call if element is an object
    } else if (typeof value === 'string') {
      // Run string conversions
      let result = value.replace(new RegExp(linefeeds.join('|'), 'g'), '~LINEBREAK~');
      result = escapeHtml(result); // Custom function to escape HTML

      // Encoding plus, ampersand, and percent signs
  ...
  for (const key in inputObject) {
    if (inputObject.hasOwnProperty(key)) {
      inputObject[key] = processValue(inputObject[key]);
    }
  }
  return inputObject;
}

// Function to replace special HTML characters
...
function webconnectReplaceStringsShared(complexArray) {
  const linefeeds = ["\r\n", "\n", "\r", "\n\r"];

  // Define output array
  return complexArray.map((v) => {
    if (Array.isArray(v)) {
      return webconnectReplaceStringsShared(v); // Recursive call if element is an array
    } else if (typeof v === 'string') {
      // Run string conversions
      v = v.replace(new RegExp(linefeeds.join('|'), 'g'), '~LINEBREAK~');
      v = escapeHtml(v); // Custom function to escape HTML
