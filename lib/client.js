...
Client.prototype.logXML = function methodCall(logFolderLocation) {
  this.logs.setLogLocation(logFolderLocation);
}
...
Client.prototype.methodCall = async function methodCall(method, params) {
  ...
  delete options.headers['Accept-Charset'];
  ...
  return new Promise((resolve, reject) => {
    ...
    response.on('data', function (chunk) { 
      this.logs.saveResponseLogs(chunk);
      body.push(chunk);
     }.bind(this))
...
Client.prototype.listLocations = async function getLocations(headers, methodName) {
  let options = {
    header: {header: headers},
    parameters: {}
  }
  let response = await this.methodCall(methodName, options);
  if (!response['data']) {
    throw 'Unable to fetch locations list.';
  }
  this.locations.set(response.data);
  return response.data;
}

Client.prototype.pullWebDRE = async function getLocations(headers, methodName, parameters) {
